...
%         \hline
%                & \multicolumn{2}{c|}{DOROS} & \multicolumn{2}{c}{arcBPM} \\
%         \cline{2-5}
% 		Method & $\sigma_{\mathrm{vis}}$ [$\mu b$] & Uncertainty (\%) & $\sigma_{\mathrm{vis}}$ [$\mu b$] & Uncertainty (\%) \\
%         \hline
...
\begin{figure}[!htb]
	\centering
	\makebox[\textwidth]{\includegraphics[width=0.3\paperwidth]{images/assets/luminous_region_vs_2d_rate_fit.png}}
...
\begin{figure}[!htb]
	\centering
	\makebox[\textwidth]{\includegraphics[width=0.6\paperwidth]{images/assets/all_Corr_luminometer_xsec.pdf}}
...
% In the previous section, we discussed the process by which the detector calibration factors were obtained. In this section, I will evaluate the performance of these calibrations in maximizing luminosity agreement between luminometers during the vdM fill and, more importantly, under physics conditions. A discussion on the year-long detector consistency will be presented, along with details on the specific stability corrections applied to the BCM1F. Additionally, the issue of detector non-linearity will be addressed, with a focus on the PLT corrections. Finally, this section will present the final integrated luminosity for 2023, along with the associated relative uncertainty.
% \newpage

% \subsection{vdM Consistency}

% Using the calibration values shown in \autoref{tab:xsec_summary}, the measured luminnosity of every independently calibrated detector can be compared in order to visualize the detector agreement at vdM conditions. \autoref{fig:vdm_instantaneous_lumi_vs_time} shows the instantaneous luminosity throught the entire vdM program.

% \begin{figure}[!htb]
% 	\centering
% 	\makebox[\textwidth]{\includegraphics[width=0.6\paperwidth]{images/assets/vdm_instantaneous_lumi_vs_time.pdf}}
% 	\caption{Measured instantaneous luminosity as a function of time during the vdM program. All independently calibrated luminometers are present and show good agreement specially in the head-on periods marked by the light grey areas.}
% 	\label{fig:vdm_instantaneous_lumi_vs_time}
% \end{figure}

% The luminometers show very good agreement between each other. The cross-detector consistency at vdM was evaluated from the head-on ratios of all the luminomenters with respect to their average. \autoref{fig:vdm_head_on_lumi_vs_time_and_ratio} shows the aggreement of the luminomenters at these periods in the vdM fill. As an uncertainty, the overall spread of the detectors was taken as the standard deviation of the means with a maginute of 0.16\% (see \autoref{fig:vdm_ratio_histograms}).

% \begin{figure}[!htb]
% 	\centering
% 	\makebox[\textwidth]{\includegraphics[width=0.7\paperwidth]{images/assets/vdm_head_on_lumi_vs_time_and_ratio.pdf}}
% 	\caption{On the right, head-on instantaneous luminosity measurements provided by the individual independently calibrated luminometers BCM1F, BCM1FuTCA, PLT, PCC, HFOC and HFET during the LHC fill 8999 as a function of time.}
% 	\label{fig:vdm_head_on_lumi_vs_time_and_ratio}
% \end{figure}

% The data in \autoref{fig:vdm_head_on_lumi_vs_time_and_ratio} corresponds to the periods when the LHC beams were colliding head-on, with all six luminometers taking high quality data, and for the sum of the nine BCID’s for which PCC data was recorded in zero bias events. Each point corresponds to a time window of 15 lumi sections (LS), with a lumi section spanning over about 23 seconds. The ratio to the average measured luminosity is shown on the right and depicts an agreement well below 1\%. The statistical uncertainty associated with each point is found to be negligible and is not shown in the plot.

% % \begin{table}[!htb]
% % 	\caption{
% % 		Integrated luminosity in head-on periods during vdM fill.
% % 	}
	
% % 	\label{tab:vdm_consistency_summary}
% % 	\centering
% % 	\begin{tabular}{lcccc}
% % 		\hline
% % 		Luminometer & Integrated luminosity [$\mu b^{-1}$] & Ratio to average & RMS \\
% % 		\hline
% % 		BCM1F     & 912.780  & 0.9979 & 0.0009 \\
% % 		BCM1FUTCA & 917.280  & 1.0028 & 0.0007 \\
% % 		HFET      & 914.732  & 1.0000 & 0.0007 \\
% % 		HFOC      & 913.588  & 0.9988 & 0.0005 \\
% % 		PLT       & 915.674  & 0.0010 & 0.0013 \\
% % 		PCC       & 914.265  & 0.9995 & 0.0008 \\
% % 		\hline
% % 	\end{tabular}
% % \end{table}
...
% With the vdM calibrations validated they can be used to to calibrate the rates of all luminometers for the entire running year in accordance to \autoref{eq:inst-luminosity}. The results of this calibration procedure are shown in \autoref{fig:vdm_only_year_stability}. 

% \begin{figure}[!htb]
% 	\centering
% 	\makebox[\textwidth]{\includegraphics[width=0.7\paperwidth]{images/assets/vdm_only_year_stability.pdf}}
% 	\caption{The left shows the ratios of the luminosity measured in time windows of 50 LS (about 20 min) between the different luminometers and the average luminosity among them as a function of the integrated luminosity. The ratio distributions are shown in a histogram on the right and the associated average and standard deviations of the means are shown.}
% 	\label{fig:vdm_only_year_stability}
% \end{figure}

% Three main observations can be made when analysing the ratios in \autoref{fig:vdm_only_year_stability}. The most obvious is the high discrepancy between different luminometers. PLT stands as a clear example of this but in general the luminometers do not show good agreement. Secondly, some detectors show big steps along the year. The most noticable examples are BCM1F at the beggining and HFET from $20.5fb^{-1}$ to $22fb^{-1}$ and $27fb^{-1}$ to $27fb^{-1}$. Lastly, the ratios show some non-linear response, as is clearly seen in PLT. These observations are artifacts that symbolize the difficulty in extrapolating the calibrations outside the vdM fill, as explained in \autoref{subsec:extrapolation_of_vdM_calibration}.

% In the next sections, the issues of detector stability and non-linearity will be addressed with examples for these corrections for the BCM1F and PLT detector.
...
% The loss of efficiency that a detector shows throughtout the year is corrected imperically by multiplying the detector's visible cross section by a factor $\epsilon$, as explained in \autoref{eq:luminosity_integration}. The way these factors are obtained is explained.

% Many fills taken during the year 2023, had designated beam time to perform emittance scans. From these scans, the detectors visible cross-sections is continuosly measured allowing for the tracking of the detectors efficiency throughtout the year. The loss of efficiency a detectors experiences will be encoded as gradually decreasing $\sigma_{\mathrm{vis}}$ measurements. When susequent measurements do not follow the expected trend, it usually indicates that the detector in question is operating under non-optimized conditions. When this happends, the detector's running configurations are changed in order to bring it back to normal operation conditions.

% When extrapolating the vdM calibration to other fills, a linear regression is performed to the FOM values as a function of integrated luminosity. The fitted line is then normalized to at vdM since this represents our best calibration measurement. \autoref{fig:efficiency_bcm1f_example} shows the fit performed for the BCM1F measurements. The big step in the beggining of \autoref{fig:efficiency_bcm1f_example} corresponds to a period in which this BCM1F charge thresholds were set so high that it was counting noisy as hits, leading to the apparent increase in efficacy.

% \begin{figure}[!htb]
% 	\centering
% 	\makebox[\textwidth]{\includegraphics[width=0.7\paperwidth]{images/assets/efficiency_bcm1f_example.pdf}}
% 	\caption{The results of emittance scan analysis for the BCM1F detector. The FOM values are shown as a function of run 3 integrated luminosity. There are 2 distinct regions that fitted independently and the fit parameters are present in the legend. }
% 	\label{fig:efficiency_bcm1f_example}
% \end{figure}

% Once the fits are performed, the fit values at every point are used to extrapolate te vdM calibrations for the corresponding fills.
...
% As in \autoref{subsec:vdm_consistency} we show the year long detector stability. Once again, the best visible cross-sections are used and the stability and non-linearity corrections have been applied.

% \begin{figure}[!htb]
% 	\centering
% 	\makebox[\textwidth]{\includegraphics[width=0.7\paperwidth]{images/assets/final_preliminary_year_stability.pdf}}
% 	\caption{The left plot shows the ratios of the luminosity measured in time windows of 50 LS (about 20 min) between the different luminometers and the reference luminometer as a function of the integrated luminosity over 2023 pp data taking when the reference luminometer provided high quality data. On the right, the ratio distributions are shown with the associated average and standard deviation of the means.}
% 	\label{fig:final_preliminary_year_stability}
% \end{figure}

% Comparing to \autoref{fig:vdm_only_year_stability} to \autoref{fig:final_preliminary_year_stability} one can observe a clear improvement in detector agreement. The standard deviation of the means went from $2.6\%$ to $0.71\%$ and represents the uncertainty associated to the stability corrections.

% To assign an uncertainty to any residual non-linearity in the detectors the slopes are extracted for all fills, as explained in \autoref{subsec:detector_non_linearity}, and resulting the distributions are analysed. \autoref{fig:year_slopes_final} shows these distributions. 

% \begin{figure}[!htb]
% 	\centering
% 	\makebox[\textwidth]{\includegraphics[width=0.7\paperwidth]{images/assets/year_slopes_final.pdf}}
% 	\caption{Distributions of relative colinearity slope values between different luminometers and the DT luminometer taken as reference (REF). Each entry in the histograms is weighted with the integrated luminosity of the fill. The left plot shows a stacked distribution while the right plot shows individual distributions. The means and standard deviations of each distribution are shown.}
% 	\label{fig:year_slopes_final}
% \end{figure}

% The maximally distant detector, HFET, is chosen for assigning the uncertainty to this step. HFOC was not considered as it is known to suffer from complex non-linearity effects which were not corrected for and, as such, does not represent our lack of understanding. The $0.09\%/\text{SBIL}$ value is then multiplied by the average SBIL in 2023 of 6.6 resulting in the overall uncertainty of $0.59\%$.

% Some luminometers are not available for certain periods throughtout the year. For this reason, all the available luminometers are given a priority for when their measured luminosity should be considered in the integrated luminosity. For this analysis the order was DT, PCC, HFET, PLT and BCM1FUTCA. For these detectors, their absolute and relative share in both the total delivered and CMS recorded luminosity was measured and are shwon in \autoref{tab:integ_shares}. The total delivered luminosity was 32.74 $fb^{-1}$ and the CMS recorded was 30.12 $fb^{-1}$ with an associated uncertainty of 1.28\%, as summarized in \autoref{}.

% \begin{table}[!htb]
% 	\centering
% 	\label{tab:integ_shares}
% 	\caption{
% 		Total luminosity shares between luminometers, for the total delivered luminosity and the recorded %GoldenJSON
% 		luminosity.
% 		The contributions depend on the priority list and each detector measurement availability.
% 	}
% 	\begin{tabular}{lcccc}
% 	\multirow{2}{*}{Luminometer} & \multicolumn{2}{c}{Total delivered} & \multicolumn{2}{c}{Recorded } \\ % GoldenJSON
% 			   & Abs.\ share [fb\textsuperscript{--1}] & Rel.\ share [\%] & Abs.\ share [fb\textsuperscript{--1}] & Rel.\ share [\%] \\ \hline
% 	DT         &  31.641 & 96.65 & 29.843 &  99.08 \\
% 	PCC        &  0.283  & 0.87 & 0.248 &   0.82 \\
% 	HFET       &  0.570  & 1.74 & 0.022 &   0.07 \\
% 	PLT        &  0.241  & 0.74 & 0.006 &  0.02 \\
% 	BCM1F uTCA &  0.002  & 0.01 & 0.000 &   0.00 \\
% 	\end{tabular}
% \end{table}

% \begin{table}[!h]
% 	\centering
% 	\caption{Summary of uncertainty contributions for the 2023 luminosity calibration analysis.}\label{tab:sum}
% 	\begin{tabular}{lc}
...
% 	%    Cross-detector linearity & 0.65 \\
% 	%(HFOC)    Cross-detector linearity & 1.25 \\
% 		Cross-detector linearity & 0.59 \\ %HFET
% 		\hline \hline
% 		Calibration & 0.89 \\
% 	%    Integration & 0.96 \\
% 	% (with HFOC)    Integration & 1.44 \\
...
\begin{figure}[!htb]
    \centering
    \makebox[\textwidth]{\includegraphics[width=0.7\paperwidth]{images/assets/year_slopes_final.pdf}}
    \caption{Distributions of relative collinearity slope values between different luminometers and the DT luminometer taken as reference (REF). Each histogram entry is weighted by the integrated luminosity of the fill.}
    \label{fig:year_slopes_final}
\end{figure}
...
The detector with the maximum deviation, HFET, is chosen to assign the uncertainty for this step. HFOC is excluded due to its complex non-linearity effects which were not corrected and thus do not represent the current understanding. The value of $0.09\%/\text{SBIL}$ is multiplied by the average SBIL in 2023 of 6.6, resulting in an overall uncertainty of $0.59\%$.
...
% \begin{table}[!htb]
%     \centering
%     \caption{Total luminosity shares between luminometers for the total delivered luminosity and the recorded luminosity. The contributions depend on the priority list and each detector's measurement availability.}
%     \label{tab:integ_shares}
%     \begin{tabular}{lcccc}
%         \multirow{2}{*}{Luminometer} & \multicolumn{2}{c}{Total delivered} & \multicolumn{2}{c}{Recorded} \\ 
%                                      & Abs.\ share [fb\textsuperscript{--1}] & Rel.\ share [\%] & Abs.\ share [fb\textsuperscript{--1}] & Rel.\ share [\%] \\ \hline
%         DT                           & 31.641 & 96.65 & 29.843 & 99.08 \\
%         PCC                          & 0.283  & 0.87  & 0.248  & 0.82  \\
%         HFET                         & 0.570  & 1.74  & 0.022  & 0.07  \\
